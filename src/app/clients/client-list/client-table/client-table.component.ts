 ...
 * @description :ClientTableComponent Represent normal view of table with all fields.
 */
...
import { SortDirection, IconSortingUpClass, IconSortingDownClass, IconSortingUpDownClass, LangaugeType, Route, RecordPerPage, CdkOverlayPane, MainDiv, CommonCssClass } from '../../../core/magic-string/common.model';
import { ColumnArray, Clients, SearchItemsArray, ClientModel } from '../../client-model';
import { ScrollService } from '../../../core/services/scroll.service';
import { GlobalEventsManager } from '../../../core/navbar/globalEventsManager';

@Component({
  selector: 'trigger-client-table',
  ...
  public ngOnChanges(): void {
    ...
    if (this.isArrayValue) {
      if (this.clients.length > 0) {
        if (this.searchText != '') {
          this.clients = this.searchPipePipe.transform(this.clients, this.searchText, SearchItemsArray)
        }
  ...
  */
  searchClients(value) {
    if (!value) {
      ...
      filteredItems = this.searchPipePipe.transform(filteredItems, value, SearchItemsArray)
